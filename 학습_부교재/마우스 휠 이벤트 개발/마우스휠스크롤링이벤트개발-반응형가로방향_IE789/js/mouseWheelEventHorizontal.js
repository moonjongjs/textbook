(function($, window, document, undefined){
	...
	var fontS = sectionW*0.078822911;
		...
		function wheelFesizeFn(){
			...
			$('html,body').stop().animate({ scrollLeft: sectionW*idx  }, 600);
		}
				...
				$(this).on('mousewheel DOMMouseScroll', function(event){
					...
					if( wheelDelta < 0){ //다음섹션 우측이동
						 if( index < $('.section').length-1 ){ //섹션10개-1까지만 수행
							 wheelMove = $(this).next().offset().left;
							 //wheelMove = $('.section').eq(index).next().offset().left;
							 $('html,body').stop().animate({ scrollLeft: wheelMove }, 600);						
							 idx = index+1;							 
						 }		
					}
					else{                //이전섹션 우측이동
						if( index > 0 ){ //처음섹션 이전까지
							wheelMove = $(this).prev().offset().left;
